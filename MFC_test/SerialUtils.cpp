...
typedef struct CommParam{
	BOOL bComOpen;
	BOOL bWrite;
	HANDLE hComm;
	HANDLE hPostMsgEvent;	
	CWinThread *pRecvThread;//声明一个接收线程
	OVERLAPPED osRead;
	OVERLAPPED osWrite;
...
int OnCommRecv(CommParam *pCP)
{
	...
	if (callBack != NULL)
		(*callBack)(pCP->serPortPar.lpParam1,buf,dwLength);	
	return 1;
}
...
UINT CommRecvProc(LPVOID pParam)
{
	...
	while (pCP->bComOpen)
	{
		...
		if (comStat.cbInQue)
		{
			...
			//PostMessage(pDlg->m_hWnd,WM_RECVCOMMNOTIFY,EV_RXCHAR,NULL);
			OnCommRecv(pCP);
			continue;
		}
		dwMask = 0;
...
UINT OnCommSend(LPVOID pParam)
{
	...
	if (callBack != NULL)
	{
		if (nSendSize < pCP->serPortPar.nDataBufSize)
			(*callBack)(pCP->serPortPar.lpParam1,1);
		else
			(*callBack)(pCP->serPortPar.lpParam1,0);
	}
...
BOOL SendComm(SerPortPar *pSerPortPar)
{
	...
	//ComPar[nPort].serPortPar.lpParam1 = pSerPortPar->lpParam1;
